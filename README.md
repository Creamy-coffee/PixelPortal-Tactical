# PixelPortal: ESP32 ESP32小地图 & 乐式(lemesh)米家智能灯无网关控制 🛰️💡
...
* **[Squaremap](https://github.com/jpenilla/squaremap)**：感谢 Squaremap 开发团队提供的强大地图渲染模组。本项目通过 Python 中间件高效抓取其生成的瓦片数据实现地图展示 。
...
│   ├── app.py            # 核心适配器：Squaremap 地图处理与 UDP 数据推送 
│   ├── deploy.sh         # 部署脚本
│   └── Dockerfile        # 容器化部署支持
├── Mod version/          # [开发中] 高效mod版本目前只有1.20.1版本的测试版且性能不佳
├── Mincreft_server/      # 插件配置参考
|   ├── comfig.yml        # Squaremap 渲染器推荐配置文件 [cite: 1]
│   └── squaremap-paper-mc1.21.10...jar # 推荐模组版本
├── PCB/                  # 硬件电路工程文件
│   └── HUB75.eprj        # 矩阵屏连接工程 (立创 EDA)
├── docs/                 # 项目媒体资源 (包含运行效果图)
└── platformio.ini        # Platform硬件库依赖管理 (ESP32-HUB75-MatrixPanel-I2S-DMA 等)
```
...
* **实时渲染推送**：Python 中间件自动计算玩家坐标瓦片，并使用 UDP 分片协议高效推送至屏幕 [cite: 15, 26, 32]。
* **实时更新 UI**：屏幕实时同步玩家坐标红点、朝向以及实时生命值效果 [cite: 25, 27, 32]。
* **多级缩放**：支持 16格 到 128格 的实时缩放分辨率调整 [cite: 16, 24, 27]。
...
* **内存管理**：**务必**将服务器的 `MaxRAM` 限制在 **(总物理内存 - 1.5G)** 以下。Squaremap 渲染地图会占用大量内存。
...
* 视图限制**：目前受限于 Squaremap 渲染稳定性，系统强制使用 `surface` (地表) 视图展示 。

---

## ⚠️ 开发者声明 (Current Status)
1. **Cave 模式**：虽有接口预留，但目前存在 Squaremap 渲染稳定性问题，**Cave 模式暂未解决** 。
2. **Mod Version**：包含了我自研的本地 Mod 源码。目前**存在已知 BUG**，仅供学习参考。
3. **AI 辅助开发声明**：本项目的代码逻辑优化、文档撰写以及部分 Bug 修复方案是在 AI 的辅助下完成的。AI 的参与极大地提升了开发效率，在此予以声明。
